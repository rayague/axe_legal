...
// Fonction pour demander un input utilisateur
...
async function seedLegalCategories() {
  console.log('🔐 Authentification requise pour le seeding...\n');
  ...
  try {
    ...
    const existingDocs = await getDocs(collection(db, 'legalCategories'));
    
    if (!existingDocs.empty) {
      console.log(`⚠️  ${existingDocs.size} catégorie(s) existe(nt) déjà.`);
      const shouldContinue = await question('Voulez-vous continuer et ajouter les nouvelles catégories ? (o/n): ');
      if (shouldContinue.toLowerCase() !== 'o') {
        console.log('❌ Opération annulée.');
        process.exit(0);
      }
    }

    console.log('\n🌱 Début du seeding des catégories juridiques...\n');
    ...
    console.log('\n💡 Vous pouvez maintenant gérer ces catégories dans Admin > Settings');
