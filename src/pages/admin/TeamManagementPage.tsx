...
export default function TeamManagementPage() {
  ...
  const handleCreateMember = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await fetch('http://localhost:4000/api/admin/team', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateMember = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`http://localhost:4000/api/admin/team/${selectedMember.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const deleteMember = async (id: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:4000/api/admin/team/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const toggleActive = async (id: number, isActive: boolean) => {
    try {
      const response = await fetch(`http://localhost:4000/api/admin/team/${id}/toggle`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      toast({
        title: isActive ? 'Membre désactivé' : 'Membre activé',
        description: `Le membre est maintenant ${!isActive ? 'visible' : 'masqué'} sur le site`,
      });
    } catch (error) {
      toast({
        title: 'Erreur',
